...
| `00_System/Startup_Guide.md` | Cold-boot procedure, auto-start table, troubleshooting |
...
| **Ollama** | `/usr/local/bin/ollama` (v0.31.1) | `~/.ollama/` — models + config |
| **nova-tools** | `~/nova-tools/.venv/bin/python` (uv venv) | `~/.nova/config.yaml` — central config |
| **MCP Server** | `~/nova-tools/mcp_server.py` (SSE :8765) | systemd: `nova-tools.service` |
| **Nova API** | `~/nova-tools/nova_api.py` (FastAPI :8766) | systemd: `nova-api.service` |
| **NovaHub** | `~/nova-tools/novahub/` (Next.js 16) | systemd: `novahub.service` |
| **deploy.sh** | `~/nova-tools/deploy.sh` | One-command redeploy |
...
| MCP Server | `8765` | SSE | ✅ Running — vault_read/write/append, semantic_search, plugins |
| Nova API | `8766` | HTTP | ✅ Running — 15 endpoints, FastAPI sidecar |
| Open Notebook | `8888` | HTTP | ❌ Not yet installed |
| UFW Firewall | — | — | ✅ Active — ports 11434 + 3000 + 8765 + 8766 open |
...
- Remote: `git@github.com:RavehnL/nova-vault.git` (private, SSH)
- Auto-commit after every write (daemon pending — manually commit for now)
